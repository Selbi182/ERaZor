...
; ===========================================================================
; ---------------------------------------------------------------------------
; Custom optimized HBlank routines
; ---------------------------------------------------------------------------
; (c) 2024 Vladikcomper
; ---------------------------------------------------------------------------
...
HBlank_WaterSurface:
	...
	lea	VDP_Data, a1
	move.w	#$8A00|$DF, VDP_Ctrl-VDP_Data(a1)	; Reset HInt timing
	...
	@transferColors:
		...
		addi.w	#$C000,d0
		swap	d0
		move.l	d0, VDP_Ctrl-VDP_Data(a1)	; write to CRAM at appropriate address
...
HBlank_Bars_HardwareOptimized:
	...
	move.w	#$8720, VDP_Ctrl-VDP_Debug(a0)			; restore backdrop color
	move.w	BlackBars.SecondHCnt, VDP_Ctrl-VDP_Debug(a0)	; send $8Axx to VDP to set HInt counter for the second invocation
	move.w	#@ToBottom, HBlankSubW				; handle bottom next time
	sf.b	VDPDebugPortSet
	assert.l a0, eq, #VDP_Debug
	move.l	(sp)+, a0
	rte
